...
function bindUpload() {
  ...
  document.getElementById('flist').addEventListener('click', e => {
    ...
    const decBtn = e.target.closest('.qty-btn.qty-dec[data-file-id]');
    const incBtn = e.target.closest('.qty-btn.qty-inc[data-file-id]');
    if (decBtn) adjustFileCopies(decBtn.dataset.fileId, -1);
    if (incBtn) adjustFileCopies(incBtn.dataset.fileId, +1);
  });
}
...
function renderFileList() {
  ...
  flist.innerHTML = files.map(f => `
    <div class="file-card" id="fc-${esc(f.id)}">
      <div class="file-preview${isColor ? '' : ' bw'}">
        <span class="fcard-preview-icon">📄</span>
        <span class="file-preview-badge">${esc(f.name.split('.').pop().toUpperCase())}</span>
      </div>
      <div class="file-info">
...
function bindPrintOptions() {
  ...
  document.querySelectorAll('.option-btn[data-color]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      renderFileList(); // تحديث معاينة B&W
    });
  });
  ...
  document.querySelector('.pkg-btn[data-pkg="none"]')?.click();
...
function updateInvoice() {
  ...
  const rows = [
    ['طباعة + تغليف', formatPrice(totals.subtotal - (customerState.get('cart') ?? []).reduce((s,i) => s + i.price * i.qty, 0))],
    ['قرطاسية', formatPrice((customerState.get('cart') ?? []).reduce((s,i) => s + i.price * i.qty, 0))],
    ['🚚 توصيل', totals.deliveryFee === 0 ? '🎁 مجاني' : formatPrice(totals.deliveryFee)],
  ];
...
function renderOrders() {
  ...
  box.querySelectorAll('.ocard').forEach(card => {
    card.addEventListener('click', () => showOrderDetail(card.dataset.oid));
  });
}
...
function showOrderDetail(orderId) {
  ...
  document.getElementById('det-title').textContent = `طلب #${orderId}`;
