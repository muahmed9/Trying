...
function navigateTo(page) {
  ...
  const titles = { orders: 'الطلبات', stats: 'الإحصائيات', dash: 'لوحة متابعة الموظفين', market: 'قرطاسية الشاطر', supplies: 'المخزن', reports: 'التقارير', settings: 'الإعدادات' };
...
async function createCoupon() {
  ...
  const scope  = document.getElementById('cpn-scope')?.value ?? 'all';
...
async function loadStaffDashboard() {
  ...
  if (!page) return;
  page.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-muted);">⏳</div>';
  try {
    const { sb: sbInst } = await import('./core/supabase.js');
    const { data: profiles } = await sbInst
      .from(Config.TABLES.PROFILES)
      .select('id, name, emoji, role, permissions')
      .neq('role', 'admin')
    ...
    page.innerHTML = `
      ...
      <div class="stats-grid" style="margin-bottom:20px;">
        <div class="stat-card"><div class="stat-num" style="color:var(--navy);">${(profiles??[]).length}</div><div class="stat-lbl">عدد الموظفين</div></div>
        <div class="stat-card"><div class="stat-num" style="color:var(--orange);">${orders.filter(o=>['received','printing'].includes(o.status)).length}</div><div class="stat-lbl">طلبات معلقة</div></div>
        <div class="stat-card"><div class="stat-num" style="color:var(--green);">${orders.filter(o=>new Date(o.created_at).toDateString()===today&&o.status==='delivered').length}</div><div class="stat-lbl">مسلَّم اليوم</div></div>
        <div class="stat-card"><div class="stat-num" style="color:var(--teal);">${orders.filter(o=>o.status==='delivering').length}</div><div class="stat-lbl">قيد التوصيل</div></div>
      </div>
      <h3 style="color:var(--navy);margin:0 0 14px;">👥 الموظفون</h3>
      ${(profiles??[]).length ? (profiles??[]).map(p => `
        <div class="staff-card" style="margin-bottom:10px;">
          ...
          <div style="font-size:.78rem;color:var(--text-muted);margin-top:6px;">
            ${(p.permissions??[]).map(x=>x.replace('→',' ← ')).join(' | ') || 'لا صلاحيات محددة'}
          </div>
        </div>`).join('') : '<p style="color:var(--text-muted);text-align:center;padding:20px;">لا يوجد موظفون بعد — أضفهم من الإعدادات</p>'}`;
  } catch(e) {
    document.getElementById('page-dash').innerHTML = `<p style="color:var(--red);text-align:center;padding:30px;">❌ ${esc(e.message)}</p>`;
  }
}
...
async function loadMarketPage() {
  ...
  // تنبيه المخزون المنخفض في صفحة القرطاسية
  const lowProds = products.filter(p => p.stock <= (p.min_stock ?? 0) && p.active);
  if (lowProds.length) {
    const alertDiv = document.createElement('div');
    alertDiv.className = 'alert-low';
    alertDiv.innerHTML = `<p class="alert-low-title">⚠️ ${lowProds.length} منتجات مخزونها منخفض</p>
      ${lowProds.map(p => `<span style="font-size:.82rem;margin-left:8px;">• ${esc(p.name)} (${p.stock})</span>`).join('')}`;
    document.getElementById('products-list-admin')?.before(alertDiv);
  }

  document.getElementById('page-market').querySelector('#products-list-admin')
...
function showProductForm(product = null) {
  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      <div class="modal-box" style="max-width:480px;">
        <h3 id="prod-form-title">➕ منتج جديد</h3>
        <input type="hidden" id="prod-form-id">
        <input type="text"   id="prod-form-name"  placeholder="✍️ اسم المنتج">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px;">
          <div>
            <label style="font-size:.75rem;color:var(--text-muted);font-weight:700;display:block;margin-bottom:4px;">السعر (د.ع)</label>
            <input type="number" id="prod-form-price" min="0" style="margin-bottom:0;">
          </div>
          <div>
            <label style="font-size:.75rem;color:var(--text-muted);font-weight:700;display:block;margin-bottom:4px;">المخزون</label>
            <input type="number" id="prod-form-stock" min="0" style="margin-bottom:0;">
          </div>
          <div>
            <label style="font-size:.75rem;color:var(--text-muted);font-weight:700;display:block;margin-bottom:4px;">الوحدة</label>
            <input type="text" id="prod-form-unit" placeholder="قطعة" style="margin-bottom:0;">
          </div>
          <div>
            <label style="font-size:.75rem;color:var(--text-muted);font-weight:700;display:block;margin-bottom:4px;">حد التنبيه</label>
            <input type="number" id="prod-form-min-stock" min="0" style="margin-bottom:0;">
          </div>
        </div>
        <select id="prod-form-cat" style="margin-bottom:12px;">
          <option value="notebooks">📓 دفاتر</option>
          <option value="books">📚 كتب</option>
          <option value="stationery">✏️ قرطاسية</option>
          <option value="other">📦 أخرى</option>
        </select>
        <input type="text" id="prod-form-img" placeholder="🖼️ رابط الصورة (اختياري)">
        <label style="display:flex;align-items:center;gap:8px;font-size:.88rem;cursor:pointer;margin-bottom:14px;">
          <input type="checkbox" id="prod-form-active" checked style="width:auto;margin:0;padding:0;accent-color:var(--teal);">
          منتج نشط (يظهر للزبائن)
        </label>
        <div style="display:flex;gap:10px;">
          <button id="prod-form-save" class="btn-primary btn-action" style="background:var(--navy);color:#fff;flex:2;" data-loading-text="جاري الحفظ ⏳...">💾 حفظ</button>
          <button id="prod-form-cancel" class="btn-primary" style="background:var(--input-bg);color:var(--text-muted);flex:1;">إلغاء</button>
        </div>
      </div>`;
    document.body.appendChild(modal);
    document.getElementById('prod-form-cancel').addEventListener('click', () => Modal.close('product-form-modal'));
    document.getElementById('prod-form-save').addEventListener('click', () => withLoading('prod-form-save', saveProductForm));
    modal.addEventListener('click', e => { if(e.target === modal) Modal.close('product-form-modal'); });
  }

  document.getElementById('prod-form-title').textContent = isEdit ? '✏️ تعديل المنتج' : '➕ منتج جديد';
  ...
  document.getElementById('prod-form-unit').value        = product?.unit ?? 'قطعة';
  document.getElementById('prod-form-min-stock').value   = product?.min_stock ?? 0;
  document.getElementById('prod-form-cat').value         = product?.category ?? 'other';
  document.getElementById('prod-form-img').value         = product?.image_url ?? '';
  document.getElementById('prod-form-active').checked    = product?.active ?? true;
...
async function saveProductForm() {
  ...
  try {
    await saveProduct({
      id:         id || undefined,
      name,
      price:      Number(document.getElementById('prod-form-price').value) || 0,
      stock:      Number(document.getElementById('prod-form-stock').value) || 0,
      unit:       document.getElementById('prod-form-unit').value || 'قطعة',
      min_stock:  Number(document.getElementById('prod-form-min-stock').value) || 0,
      category:   document.getElementById('prod-form-cat').value,
      image_url:  document.getElementById('prod-form-img').value.trim() || null,
...
function adjProductStock(id, products) {
  ...
  adminState.set('_txSupplyType', 'set');
  document.getElementById('tx-type-label').textContent  = '📦 تعديل مخزون المنتج';
  document.getElementById('tx-supply-name').textContent = p.name + ` (حالي: ${p.stock})`;
  document.getElementById('tx-qty').value  = p.stock;
  document.getElementById('tx-notes').value = '';
  // استخدام tx-confirm-btn لحفظ مخزون المنتج
  adminState.set('_txIsProduct', true);
  adminState.set('_txProductId', id);
...
async function loadSuppliesPage() {
  ...
  // event delegation — آمن من تراكم listeners
...
async function confirmSupplyTx() {
  ...
  // مخزون منتج قرطاسية
  if (adminState.get('_txIsProduct')) {
    try {
      await adjustProductStock(adminState.get('_txProductId'), 'set', qty);
      adminState.set('_txIsProduct', false);
...
function loadReportsPage() {
  // استخدام onclick لتجنب تراكم الـ event listeners عند كل زيارة
