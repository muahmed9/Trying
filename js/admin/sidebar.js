...
export class Sidebar {
  ...
  // ── بناء عنصر القائمة ────────────────────────
  #pageItem(page) {
    const badge = page.id === 'orders'
      ? '<span class="sidebar-badge" style="display:none;">0</span>'
      : '';
    return `
      <button class="sidebar-item" data-page="${page.id}">
        <span class="sidebar-item-icon">${page.icon}</span>
        <span class="sidebar-item-text">${page.label}</span>
        ${badge}
      </button>`;
  }
