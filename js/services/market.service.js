/**
 * market.service.js — المتجر والمخزن
 */
...
export async function adjustProductStock(id, type, qty) {
  const { data: prod, error: fetchErr } = await sb
    .from(T.MARKET_PRODUCTS).select('stock').eq('id', id).single();
  ...
  const { error } = await sb.from(T.MARKET_PRODUCTS)
    .update({ stock: newStock }).eq('id', id);
  if (error) throw error;
}
...
export async function fetchSupplies() {
  const { data, error } = await sb
    .from(T.SUPPLIES).select('*').order('name');
  if (error) throw error;
  return data ?? [];
}
...
export async function adjustSupplyStock(id, type, qty, note = '') {
  const { data: sup, error: fetchErr } = await sb
    .from(T.SUPPLIES).select('stock').eq('id', id).single();
  if (fetchErr) throw fetchErr;

  const delta    = type === 'add' ? Number(qty) : -Number(qty);
  const newStock = Math.max(0, sup.stock + delta);

  const { error } = await sb.from(T.SUPPLIES)
    .update({ stock: newStock }).eq('id', id);
  if (error) throw error;

  // تسجيل في سجل المخزن
  await sb.from(T.SUPPLY_LOG).insert({
    supply_id: id, type, qty: Number(qty),
...
export async function loadPricing() {
  const { data } = await sb
    .from(T.SETTINGS).select('value').eq('key', 'pricing').maybeSingle();
  if (!data?.value) return null;
  return { ...Config.DEFAULT_PRICING, ...data.value };
}

export async function savePricing(pricing) {
  const { error } = await sb.from(T.SETTINGS)
    .upsert({ key: 'pricing', value: pricing }, { onConflict: 'key' });
  if (error) throw error;
}
