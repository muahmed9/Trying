 ...
 * يربط كل المكونات ويضع addEventListener بدلاً من onclick
 */
...
//  طلبات البحوث
// ═══════════════════════════════════════
let _researchBound = false;
function _bindResearchForm() {
  if (_researchBound) return;
  _researchBound = true;
  document.getElementById('res-btn')?.addEventListener('click', () =>
    withLoading('res-btn', sendResearchOrder)
  );
}

async function sendResearchOrder() {
  ...
  if (!isValidName(name))        { errEl.textContent = '❌ يرجى إدخال الاسم'; errEl.style.display='block'; return; }
  ...
  try {
    ...
    await sbInst.from(Config.TABLES.RESEARCH).insert({
      name, phone, subject, type,
      pages:    Number(document.getElementById('res-pages').value) || null,
      deadline: document.getElementById('res-deadline').value || null,
      details:  document.getElementById('res-details').value.trim(),
      status:   'pending',
    });
    document.getElementById('res-confirm-box').style.display = 'block';
    // إخفاء الفورم
    ['res-name','res-phone','res-subject','res-details'].forEach(id => {
      const el = document.getElementById(id); if(el) el.value = '';
    });
    document.getElementById('res-btn').style.display = 'none';
  } catch(e) {
    errEl.textContent = '❌ تعذّر إرسال الطلب — ' + e.message;
    errEl.style.display = 'block';
  }
}

// ── ملخص الطلب في الخطوة 3 ──────────────────
function renderOrderSummaryStep3() {
  ...
  let el = document.getElementById('step3-order-summary');
  if (!el) {
    const step3 = document.getElementById('step-3');
    el = document.createElement('div');
    el.id = 'step3-order-summary';
    el.style.cssText = 'margin-bottom:12px;';
    step3?.prepend(el);
  }

  const totalPages  = files.reduce((s,f) => s + (f.pages??1)*(f.copies??1), 0);
  ...
  const filesRows = files.map(f =>
    `<div style="display:flex;justify-content:space-between;font-size:.82rem;padding:4px 0;border-bottom:1px solid var(--border);">
       <span>📄 ${esc(f.name.slice(0,30))}</span>
       <span style="color:var(--text-muted);">${(f.pages??1)} ص × ${f.copies??1}</span>
     </div>`).join('');
  ...
  el.innerHTML = `
    <div class="card" style="border:2px solid var(--teal);padding:14px;">
      <b style="color:var(--navy);display:block;margin-bottom:10px;">📋 ملخص طلبك</b>
      ${files.length ? `<div style="font-size:.78rem;color:var(--text-muted);margin-bottom:6px;">إجمالي الصفحات: <b style="color:var(--navy);">${totalPages} صفحة</b> • ${color} • ${side} • ${pkg}</div>${filesRows}` : ''}
      ${cart.length  ? `<div style="margin-top:8px;">${cartRows}</div>` : ''}
      ${!files.length && !cart.length ? '<p style="color:var(--text-muted);font-size:.85rem;text-align:center;">لا يوجد محتوى بعد</p>' : ''}
    </div>`;
}
...
async function init() {
  ...
  try {
    ...
    if (data) {
      ...
      const newUser = { id: userId, name: tgU?.first_name ?? '', username: tgU?.username ?? '',
  ...
  // تحميل التسعير والمنتجات المقترحة
  const pricing = await loadPricing();
  if (pricing) customerState.set('pricing', pricing);

  // إظهار هدية أول طلب — فقط إذا انتهى onboarding مسبقاً (جلسة سابقة)
...
function goTab(t) {
  ...
  if (t === 'research') _bindResearchForm();
}
...
function bindStepper() {
  stepper = new Stepper(4, step => {
    updateSummaryBar();
    if (step === 3) renderOrderSummaryStep3();
...
function bindUpload() {
  ...
  // QtyControl delegation للنسخ — data-id على .qty-ctrl wrapper
...
function renderFileList() {
  ...
  // عدادات الملفات والصفحات
  const totalPages = files.reduce((s, f) => s + (f.pages ?? 1) * (f.copies ?? 1), 0);
  const totalFiles = files.length;
  let counterEl = document.getElementById('files-counter');
  if (!counterEl) {
    counterEl = document.createElement('div');
    counterEl.id = 'files-counter';
    counterEl.style.cssText = 'background:var(--light-teal);border-radius:var(--radius-sm);padding:8px 14px;margin-bottom:10px;font-size:.85rem;font-weight:700;color:var(--navy);display:flex;gap:16px;';
    document.getElementById('flist').before(counterEl);
  }
  counterEl.style.display = files.length ? 'flex' : 'none';
  counterEl.innerHTML = `<span>📁 ${totalFiles} ملف</span><span>📄 ${totalPages} صفحة إجمالاً</span>`;
...
function bindPrintOptions() {
  ...
  document.querySelectorAll('.option-btn[data-color]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      renderFileList();
      _updateLivePricePreview(); // تحديث السعر فوراً
    });
  });
...
// ═══════════════════════════════════════
//  معاينة السعر الحية (بين الخطوات)
// ═══════════════════════════════════════
function _updateLivePricePreview() {
  const files = customerState.get('files') ?? [];
  if (!files.length) return;
  const pricing = customerState.get('pricing') ?? Config.DEFAULT_PRICING;
  const totals  = calcOrderTotals({
    files, cart: customerState.get('cart') ?? [],
    sugCart: customerState.get('suggestedCart') ?? {},
    pricing, coupon: customerState.get('appliedCoupon'),
    user: customerState.get('user'),
  });
  // عرض مؤشر سعر مبدئي في الخطوة 2
  let el = document.getElementById('live-price-preview');
  if (!el) {
    el = document.createElement('div');
    el.id = 'live-price-preview';
    el.style.cssText = 'background:var(--navy);color:#fff;border-radius:var(--radius-md);padding:10px 16px;margin-top:12px;text-align:center;font-weight:800;font-size:.95rem;';
    document.querySelector('.stepper-nav')?.before(el);
  }
  el.textContent = `💰 التقدير المبدئي: ${formatPrice(totals.total)}`;
}

// ═══════════════════════════════════════
//  نموذج الطلب (الخطوة 4)
// ═══════════════════════════════════════
  ...
  grid.innerHTML = filtered.map(p => {
    ...
    return `
      <div class="product-card" data-pid="${esc(p.id)}">
        ...
        <span class="product-price">${formatPrice(p.price)} / ${esc(p.unit ?? 'قطعة')}</span>
...
async function loadOrders() {
  ...
  try {
    ...
    // إذا كان الجدول غير موجود أو خطأ في الصلاحيات — أظهر فارغاً بدل خطأ
    console.warn('loadOrders:', e.message);
    customerState.set('allUserOrders', []);
    renderOrders();
  }
}
...
async function loadPtsTab() {
  ...
  // سجل النقاط من الطلبات المسلَّمة
  try {
